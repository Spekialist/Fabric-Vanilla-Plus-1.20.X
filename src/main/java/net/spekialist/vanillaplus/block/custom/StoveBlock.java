...
public class StoveBlock extends BlockWithEntity implements BlockEntityProvider {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!world.isClient) {
            NamedScreenHandlerFactory screenHandlerFactory = ((StoveBlockEntity) world.getBlockEntity(pos));
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.STOVE_BLOCK_ENTITY,
                (world1, pos, state1, blockEntity) -> StoveBlockEntity.tick(world, pos, state, blockEntity));
    }
